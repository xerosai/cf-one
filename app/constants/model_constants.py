# File: model_constants
# Desc: app model constants
# Date: November 30, 2016 @ 3:04 PM
# Auth: Simon Neufville (simon@xrscodeworks.com) / xerosai


class ModelConstants(object):

    COMMON_OBJECT_ID = 'object_id'
    COMMON_CREATED_AT = 'created_at'
    COMMON_UPDATED_AT = 'updated_at'
    COMMON_ASSOC_USER = 'assoc_user'

    CAFE_ITEM_NAME = 'item_name'
    CAFE_ITEM_DESC = 'item_desc'
    CAFE_ITEM_TYPE = 'item_type'
    CAFE_ITEM_COST_SPEC = 'item_cost_spec'

    ITEM_COST_SPEC_SIZE = 'spec_item_size'
    ITEM_COST_SPEC_COST = 'spec_item_cost'

    ORDERS_RELATED_USER = 'related_user'
    ORDERS_ORDER_STATUS = 'order_status'
    ORDERS_ORDER_ITEMS = 'order_items'

    USER_PROFILE_AUTH0_ID = 'auth0_id'
    USER_PROFILE_DISPLAY_NAME = 'display_name'
    USER_PROFILE_EMAIL_ADDRESS = 'email_address'
    USER_PROFILE_DISPLAY_PICTURE = 'display_picture'
